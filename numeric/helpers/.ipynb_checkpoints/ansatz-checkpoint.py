...
def f(t, alpha, g_coeffs, theta, mstar, factorial_cache_info):
    ...
    n_range_exp_higher = n_range.unsqueeze(0).unsqueeze(0).expand(B, max_M - 1, max_N)
    factorial_cache_n_exp_higher = factorial_cache_n.unsqueeze(0).unsqueeze(0).expand(B, max_M - 1, max_N)
    ...
    # gstar
    t_sub_star = (t - theta[0,0]).unsqueeze(1).expand(B, max_N)  # (B, max_N)
    t_powers = (t_sub_star ** n_range_exp) / factorial_cache_n_exp  # (B, max_N)
    
    g_coeffs_0_exp = g_coeffs[0].unsqueeze(0).expand(B, max_N)
    #theta_0_exp = theta[0].unsqueeze(0).expand(B, max_N)
    
    heaviside_theta_gstar = helper_theta(t_sub_star, 0) # (B, max_N)
    ...
    # g higher
    t_exp = t.unsqueeze(1).unsqueeze(2).expand(B, max_M - 1, max_N)  # (B, M-1, N)
    theta_1_exp = theta[1:].unsqueeze(0).expand(B, max_M - 1, max_N)
    t_sub_higher = (t_exp - theta_1_exp)  # (B, M-1, N)

    t_powers_exp = (t_sub_higher ** n_range_exp_higher) / factorial_cache_n_exp_higher  # (B, M-1, N)

    g_1_exp = g_coeffs[1:].unsqueeze(0).expand(B, max_M - 1, max_N)
    
    heaviside_theta_ghigher = helper_theta(t_sub_higher, 0)
