...
for epoch in t:
    ...
    """
    This may not be at all correct
    ...
    if MODE == "UNIFORM_SAMPLES":
        ...
        alpha_s_0 = torch.tensor([0.0], requires_grad = True)
        ...
        p = target_p(xs[:,0], alpha_s_0, E0, R)

        # send samples through the flow
        logq = flow(alpha_s_0).log_prob(inverse_sigmoid(xs)) +  logJ
        ...
        if LOSS == "klDiv":
            #loss = torch.nanmean(C_prescale(xs[:,0],c_expanded,E0,R) * torch.exp(logp) * (logp - logq) )
            loss = torch.nanmean(torch.exp(logq) * (logq - logp) )
            
        if LOSS == "residual":
            
            #print(logq,p)
            residual = torch.log((p+epsilon)/(torch.exp(logq) + epsilon) )
            #residual = 1.0 - (p / torch.exp(logq))
            ...
            print(p)
            print("q") 
            print(torch.exp(logq))
           
            print("residual") 
            print(residual)
            dResidual_dAlpha = torch.ones(residual.shape)
            print(alpha_s_0.grad)
        
            dResidual_dAlpha = torch.autograd.grad(inputs=alpha_s_0, outputs=residual, grad_outputs=dResidual_dAlpha, allow_unused=True, create_graph=True, retain_graph=True)[0]
            
            print( print(alpha_s_0.grad))

            #gradient.backward(inputs=alpha_s_0, tensors=residual, gradient=dResidual_dAlpha, retain_graph=True)
            #dResidual_dAlpha.requires_grad_()
           
            print("dResidual_dAlpha")
            print(dResidual_dAlpha)
            
            
            loss = torch.nanmean(torch.norm(dResidual_dAlpha, p=2))
            #loss_full.requires_grad_()
            #loss = torch.nanmean(loss_full)
...
for i in range(len(cs)):
    ...
    if DRAW_NF_LINES:
        ...
        for aux_draw in range(aux_draws):
            ...
            ax.vlines(cs[i], 0, target_p(c, alpha_s_0, E0, R).detach().numpy(), color = jet_color, linestyle = "--", alpha = 0.25, lw = 0.5)
...
plt.plot(xs, target_p(torch.tensor(xs), alpha_s_0, E0, R).detach().numpy(), color = "Black", label = "Target (Uncut)")
