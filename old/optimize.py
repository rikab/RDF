...
def train(epochs, batch_size, lr):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        alpha_zero = alpha_zero.clone().detach().requires_grad_(True)
        base  = fn(alpha_zero)
        d1,   = torch.autograd.grad(base, alpha_zero, create_graph=True)
        d2,   = torch.autograd.grad(d1,   alpha_zero, create_graph=True)
